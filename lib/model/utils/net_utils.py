...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        score = dets[i, -1]
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 204, 0), 6)
            cv2.putText(im, '%s: %.3f' % (class_name, score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN,
                        7.0, (0, 0, 255), thickness=10)
    return im
...
def to_json(rects, class_name, dets, thresh=0.8):
    """Visual debugging of detections."""
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            roidb["ymin"] = bbox[1]
            roidb["ymax"] = bbox[3]
            roidb["confidence"] = round(float(score), 6)
            roidb["label"] = str(class_name)
            rects.append(roidb)
    return rects
...
def change_bbox(bbox1, bbox2):
    bbox = list()
    bbox.append(min(bbox1[0], bbox2[0]))
    bbox.append(min(bbox1[1], bbox2[1]))
    bbox.append(max(bbox1[2], bbox2[2]))
    bbox.append(max(bbox1[3], bbox2[3]))
    bbox.append(max(bbox1[4], bbox2[4]))
    return bbox

def union_bbox(rects):
    rects = np.array(rects).tolist()
    for flag in range(1, len(rects)):
        for i in range(len(rects)):  # 0, 1, 2, 3
            for j in range(i + 1, len(rects)):  # 1-
                if cal_IOU(rects[i][:4], rects[j][:4]) != 0:
                    rects[i] = change_bbox(rects[i], rects[j])
                    del rects[j]
                    break
                else:
                    pass
...
def add_x_y_bbox(x, y, rects, class_name, dets, thresh=0.8):
    """Visual debugging of detections."""
    for i in range(np.minimum(10, dets.shape[0])):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
...
def change_patch_bbox(bbox1, bbox2):
    bbox = dict()
    bbox["xmin"] = min(bbox1["xmin"], bbox2["xmin"])
    bbox["ymin"] = min(bbox1["ymin"], bbox2["ymin"])
    bbox["xmax"] = max(bbox1["xmax"], bbox2["xmax"])
    bbox["ymax"] = max(bbox1["ymax"], bbox2["ymax"])
    bbox["confidence"] = max(bbox1["confidence"], bbox2["confidence"])
    bbox["label"] = [bbox1["label"], bbox2["label"]][np.random.randint(2)]
    return bbox

def union_patch_bbox(rects):
    # rects = np.array(rects).tolist()
    for flag in range(1, len(rects)):
        for i in range(len(rects)):  # 0, 1, 2, 3
            for j in range(i + 1, len(rects)):  # 1-
                if rects[i]["label"] == rects[j]["label"]:
                    i_list = [rects[i]["xmin"], rects[i]["ymin"], rects[i]["xmax"], rects[i]["ymax"]]
                    j_list = [rects[j]["xmin"], rects[j]["ymin"], rects[j]["xmax"], rects[j]["ymax"]]
                    if cal_IOU(i_list, j_list) != 0:
                        rects[i] = change_patch_bbox(rects[i], rects[j])
                        del rects[j]
                        break
                    else:
                        pass
...
def vis_bbox(im, rects):
    for rect in rects:
        cv2.rectangle(im, (rect["xmin"], rect["ymin"]), (rect["xmax"], rect["ymax"]), (0, 204, 0), 6)
        cv2.putText(im, '%s: %.3f' % (rect["label"], rect["confidence"]), (rect["xmin"], rect["ymin"] + 15), cv2.FONT_HERSHEY_PLAIN,
                    7.0, (0, 0, 255), thickness=10)
    return im
